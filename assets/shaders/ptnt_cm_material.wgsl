...
@group(1) @binding(1) var metalicRoughnessTexture : texture_2d<f32>;


@group(2) @binding(0) var colorSampler : sampler;
@group(2) @binding(1) var metalicRoughnessSampler : sampler;
...
fn fragment_main(in: VertexOut) -> @location(0) vec4f {
  let color = textureSample(colorTexture, colorSampler, in.texCoords.xy);
  let metalicRoughness = textureSample(metalicRoughnessTexture, metalicRoughnessSampler, in.texCoords.xy);
  ...
  for(var i: u32 = 0; i < arrayLength(&pointLights); i++) {
    ...
    var d = normalize(TBN * in.normals);
